...
//! python and prose go through `tests/e2e/lib/lib.sh`.
//!
...
/// Repo root: this crate sits at `<root>/tests/testrig`.
...
fn holds_no_source(dir: &Path) -> bool {
    matches!(
        ...
        Some("target" | ".git" | "__pycache__" | "website" | ".claude" | ".agents")
...
fn tmux_teardown_has_exactly_one_home_in_rust() {
    ...
    let rig = repo_root().join("tests/testrig");
...
fn tmux_teardown_has_exactly_one_home_outside_rust() {
    let lib = repo_root().join("tests/e2e/lib/lib.sh");
    let mut offenders = Vec::new();
    for path in tree() {
    ...
    assert!(
        ...
        "these files stop a tmux server themselves instead of calling \
         cyc_tmux_teardown from tests/e2e/lib/lib.sh, which is where the \
         unlink and the already-gone fallback live (python sources it \
         through bash, see tests/e2e/m1_soak.py): {offenders:#?}"
    );
}
