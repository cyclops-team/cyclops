...
tests/e2e/lib/lib.sh so the server's socket file goes with the server. Never
...
Usage: python3 tests/e2e/m1_soak.py [count-per-cli]   (default 100)
...
MANIFESTS = os.path.join(REPO, "resources", "manifests")
...
# down. Rust has tests/testrig, bash has this file, and python
# gets neither by holding a third copy.
LIB_SH = os.path.join(REPO, "tests", "e2e", "lib", "lib.sh")


def shared_rule(func, *args, env=None):
    """Run one function from tests/e2e/lib/lib.sh; return its stdout, trimmed.
...
def tmux_teardown():
    ...
    gone, lives in tests/e2e/lib/lib.sh.
    """
...
# vocabulary as tests/e2e/lib/tuikit.py.
