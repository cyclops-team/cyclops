...
# Source it, do not paste it. Every demo does, and so do
# tests/e2e/m1_soak.py and tests/e2e/lib/tuikit.py, which reach this file
# rather than hold a python copy of either rule.
#
...
# tests/testrig. Restated here because bash cannot call them;
# change them together. tests/testrig/tests/teardown_has_one_home.rs fails if
# a third home appears anywhere in the tree, and
# tests/testrig/tests/shell_teardown.rs holds this file to the
...
# back on the way down (docs/guides/panes.md), and a trap that killed and
# moved straight on to cyc_tmux_teardown raced that restore against the
# tmux server going away.
cyc_stop_daemon() {
