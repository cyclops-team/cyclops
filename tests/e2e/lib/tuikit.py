...
def tmux(*args, check=False):
    ...
    # tmux config. Same argv shape as tests/e2e/m1_soak.py.
...
def teardown():
    ...
    tests/e2e/lib/lib.sh; python calls it instead of keeping a copy,
    which tests/testrig/tests/teardown_has_one_home.rs enforces.
    """
    subprocess.run(["bash", "-c", '. "$1"; shift; "$@"', "bash",
                    os.path.join(REPO, "tests", "e2e", "lib", "lib.sh"),
...
def dismiss_modal(target, cap, log, cli=None):
    ...
    `cli` selects the per-CLI decline vocabulary (same as tests/e2e/m1_soak.py)."""
