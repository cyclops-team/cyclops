...
One probe test lives a level up, at `tests/e2e/test_vocab.py`, because it is
itself a test rather than shared machinery: modal-vocabulary regression,
no tmux needed. Locks the trust-dialog handling (explicit affirmative,
never Escape on a dialog whose text says Esc cancels/exits) against the
real Claude 2.1.220 capture. Run directly: `python3 tests/e2e/test_vocab.py`.
...
- Teardown goes through `tuikit.teardown()`, even on failure. Stopping a
  ...
  statement of that rule (`cyc_tmux_teardown` in `lib.sh`, this directory).
  A leaked server or socket is a bug in the probe.
...
`resources/manifests/*.toml` in the same change.
