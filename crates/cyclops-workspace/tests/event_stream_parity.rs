...
//! model (crates/cyclops-ui/src/stream.rs); `cyclops_workspace::
//! event_stream_rows` is the workspace panel's actual row-producing path
//! (`crates/cyclops-workspace/src/render.rs`, also called by
...
//! `Entry::lines`, `crates/cyclops-ui/src/plain.rs` `print_line`) and
...
/// per E2's scope; see `App::record`'s doc in `crates/cyclops-workspace/
/// src/app.rs`).
fn build_record() -> Record {
    ...
    //    `crates/cyclops-ui/src/stream.rs` documents on `Record::admits`.
...
/// (`crates/cyclops-ui/src/plain.rs` `print_line`, comfortable density):
