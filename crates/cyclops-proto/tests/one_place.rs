...
//! `crates/cyclops-proto/src/attention.rs`. Its predicates are
...
//!
//! This file is a text scan over `crates/**/*.rs`. It catches the common
//! shapes of a second implementation. It is NOT a proof that the rule has
//! one home, and it cannot be one: deciding whether two pieces of code
//! compute the same predicate is semantic equivalence, and no scan over
//! source text decides that.
//!
...
/// Repo root: this crate sits at `<root>/crates/cyclops-proto`.
...
fn owner() -> PathBuf {
    repo_root().join("crates/cyclops-proto/src/attention.rs")
}
...
fn agent_predicate_home() -> PathBuf {
    repo_root().join("crates/cyclops-proto/src/state.rs")
}

/// This file. It quotes the rule in both spellings on purpose.
fn here() -> PathBuf {
    repo_root().join("crates/cyclops-proto/tests/one_place.rs")
}
...
/// - Anything outside Rust under `crates/`: the frontend, the demos, the
///   docs, the shipped manifests, the python harness.
...
fn the_tripwire_is_not_a_proof_and_says_where_it_fails() {
    // The checks are text over Rust under crates/, and that is the whole
