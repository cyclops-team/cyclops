//! The demos' daemon-stop helper has one home, `demos/lib.sh`.
//!
//! It had three. `stop_daemon` was pasted byte for byte into
//! `demos/m4-name.sh`, `demos/m5-theme.sh` and `demos/parity-check.sh`
//! while the file whose header says "source it, do not paste" sat next to
//! them. Nothing was wrong with any copy, which is the problem: the next
//! change to the rule fixes one of three and leaves two behind, each green
//! on its own. Same shape as
//! `crates/cyclops-testrig/tests/teardown_has_one_home.rs`, which guards
//! the tmux half of the same file, and it lives in this crate because the
//! process being stopped is cyclopsd.
//!
...
/// Repo root: this crate sits at `<root>/crates/cyclopsd`.
...
fn stopping_the_daemon_has_exactly_one_home() {
    let root = repo_root();
    let lib = root.join("demos/lib.sh");
    ...
    assert!(
        ...
        "these files define their own daemon stop instead of calling \
         cyc_stop_daemon from demos/lib.sh, which is where the wait and the \
         safe-to-call-twice rule live: {offenders:#?}"
    );

    let text = std::fs::read_to_string(&lib).expect("read demos/lib.sh");
    assert!(
        text.contains(THE_HOME),
        "demos/lib.sh no longer defines the rule it is the home of"
    );
}
