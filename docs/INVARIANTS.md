...
- Enforced at: `crates/cyclopsd/src/delivery.rs`, `gate` (admission),
  `occupant_unchanged` (both re-checks), `attempt_delivery` (the order),
  `inject` and `staged_verified` (verification).
- Proven by: `crates/cyclopsd/tests/m1_blockers.rs`,
...
- Enforced at: `crates/cyclopsd/src/delivery.rs`, the `AgentState::
  IdleWithInput` arm of `gate`; `crates/cyclopsd/src/fusion.rs` supplies
  the escaped capture; `manifests/codex.toml` rules
...
- Enforced at: `crates/cyclopsd/src/delivery.rs`, `park_recipient`;
...
- Enforced at: `crates/cyclopsd/src/delivery.rs`, `close_limbo`; every
  transition goes through `advance`, which appends a line.
- Proven by: `crates/cyclopsd/tests/m1_fixes.rs`,
  `restart_closes_limbo_deliveries`; `crates/cyclopsd/tests/m1_blockers.rs`,
...
- Enforced at: `crates/cyclopsd/src/identity.rs`, `peer_of` and
  `resolve_sender`; `crates/cyclopsd/src/server.rs`,
  `verify_report_origin`.
- Proven by: `crates/cyclopsd/src/server.rs`,
  `msg_send_fails_closed_without_peer_credentials`;
  `crates/cyclopsd/tests/m2_hooks.rs`,
...
- Enforced at: `crates/cyclops-proto/src/ledger.rs` (schema and the rule);
  `crates/cyclopsd/src/delivery.rs`, `gate_line` and `parse_reset_hint`.
- Proven by: `crates/cyclopsd/tests/m1.rs`, which asserts the raw modal
  text and the raw quota banner are absent from the ledger;
  `crates/cyclopsd/tests/m1_blockers.rs`, same assertion on the park path.
...
- Enforced at: `crates/cyclops-ledger/src/lib.rs`, `LedgerWriter` (append
...
- Proven by: `crates/cyclopsd/tests/restart_eye.rs`,
...
- **One debounce.** `RECONCILE_DEBOUNCE`, 30ms, in
  `crates/cyclops-tmux/src/watcher.rs`. Change hints arrive in bursts (a
...
- **One-shot timers inside a delivery**, each bounded and each listed in
  the header of `crates/cyclopsd/src/delivery.rs`: the post-paste
...
- Enforced at: `crates/cyclops-tmux/src/watcher.rs` (the debounce and the
...
`hooks/`, workspace presets under `layouts/`, and theme palettes under
`themes/` are all data files, and none of them is a code path.

- Enforced at: `crates/cyclops-manifest` (parse, validate, evaluate, and
  nothing vendor-specific); `manifests/{claude,codex,agy}.toml`.
- Proven by: `crates/cyclops-manifest/tests/shipped_rules.rs`, which runs
