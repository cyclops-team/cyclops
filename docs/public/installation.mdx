...
and places the Cyclops skill in each supported skill home. It preserves
unrelated vendor settings, backs up a vendor config before its first edit, and
does not create homes for agent CLIs that are not installed. Set
...
that created the shell, then start a fresh terminal. Do not claim richer
...
Operator-edited setup files stay unchanged. Known unedited shipped themes,
manifests, hook artifacts, and skills may advance with the installed release.
Update also refreshes Cyclops-owned vendor hook entries and known unedited
skills unless `CYCLOPS_NO_VENDOR_HOOKS=1` is set.
...
Uninstall removes the two public binary links, the validated managed pair
store, and the installer-owned profile block. It refuses an altered pair store
instead of deleting an unproven tree. It leaves `~/.cyclops` in place because
that directory contains your record. Both public names are removed only from
