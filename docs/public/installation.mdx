...
- A Rust toolchain with `cargo`, recent stable (1.85+). Cyclops builds
  from source, so this is a hard requirement. Missing it? Install Rust
  with [rustup](https://rustup.rs), not Homebrew or apt: package-manager
  Rust pins the compiler it shipped with, and rustup is what keeps a
  stable toolchain current.
