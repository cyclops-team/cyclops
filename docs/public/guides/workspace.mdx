...
motion = true
terminal_default_fg = "#3a2b26" # optional exact terminal palette pair
...
palette pair lets a terminal that ignores OSC 111 restore its default colors
through an explicit OSC 10/11 set. Both values are required, and Cyclops
never queries terminal input for them.
