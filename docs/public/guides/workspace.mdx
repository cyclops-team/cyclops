...
— session `main`, one shell pane in the directory you ran it from, tabs
...
- **Tab bar** (top) — one tab per tmux window, plus `+` for the next one.
  Hide it from the app menu's `Tab bar` item.
...
agent's directory, within a second of a pane switch or `cd`; the **pinned
browser** stays where you last left it, remembered across launches
(`files_pinned_root`). The header's chip flips between them: `[pin]` while
following the agent, `[agent]` while pinned. Click a file and its path is
typed into the focused pane as `@src/main.rs `, relative to the panel's
root — nothing is submitted. A file clicked in the pinned view still
