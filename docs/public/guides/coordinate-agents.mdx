...
keeps those facts separate: `accepted` covers the mailbox record, while `wake
<state>` reports the current notification attempt.
...
<full durable body>
Reply: cyclops reply m-… --summary "First sentence. Second sentence." --body "..."
[cyclops:end m-…]
```
...
Summaryless legacy clients retain the visible full-payload compatibility path.
Current `cyclops send` and `cyclops reply` commands always use the summary and
exact-claim notification.
