...
folder_tracked = []
terminal_default_fg = "#3a2b26" # optional pair for terminals that ignore OSC 111
...
`#rrggbb` value. Cyclops restores that pair through OSC 10/11 on workspace
exit and focus loss when supplied; an absent, partial, or invalid pair uses
the terminal's OSC 110/111 reset. Cyclops does not read terminal input to
discover a palette.
