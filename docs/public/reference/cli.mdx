...
attention. Add `--require-wake` when a script also requires a healthy wake.
...
- `1`: no successful response. With `send --require-wake`, it can instead mean
  the message is durable while its optional terminal wake needs attention. A
  send may also already be durable if its response was lost.
