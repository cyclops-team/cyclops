...
| Pane-only mode has no compact visibility | Corrected by Milestone 7 without forcing one journey onto another | The collapsed full-workspace rail now projects authenticated body-free Work, attention, and stale or unknown state. Adopted tmux panes retain their unread border count, while explicitly chrome-free native tmux retains manual inbox inspection. |
...
| Replay and torn-tail behavior | `INVARIANTS.md`, `DELIVERY.md` | Newline-terminated records stay immutable; lenient replay seals an unterminated unacknowledged tail and retains it when valid, while strict replay removes only that tail and exposes mid-history corruption | Yes | Checkpoints or indexes require measurement and replay equivalence |
...
| Data lifecycle | Interim operator rule | Append and replay are specified; full retention, export, restore, deletion, and migration policy is deferred | No acknowledged record is silently deleted, truncated, or rewritten | A breaking migration requires an explicit export or migration path; a complete lifecycle policy needs separate approval |
...
| Pane-only visibility | Implemented | The collapsed workspace rail derives body-free Work and attention state from authenticated snapshots; the adopted tmux border retains its unread count | Preserve a stateful collapsed Messages rail, existing body-free tmux border count, and intentionally chrome-free mode with manual inbox inspection | Milestone 7 focused hidden-refresh and renderer regressions | A separately approved product change |
