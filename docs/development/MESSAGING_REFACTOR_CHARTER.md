...
| Runtime observation still coordinates several consequence domains | Milestone 4 moved positive quota-reset consequences behind an immutable pane observation consumed by `WorkspaceMessaging`; the Milestone 3 recovery and composer-projection corrections removed durable recovery and active-candidate interpretation from fusion. Fusion still invokes the exact-owned evidence consequence directly after a state or composer edge, emits events, paints chrome, and confirms delivery ACKs. | Verified responsibility leak, narrowed by Milestones 3 and 4; the immutable exact-owned evidence edge is the next observation correction |
