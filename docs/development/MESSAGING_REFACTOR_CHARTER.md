...
| Current acceptance is already durable-first | `server::msg_send` authenticates, `messaging::send` calls `MailboxService`, and the store prepares, appends, then commits its projection before scheduling notification, unread, and invalidation consequences. | Verified behavior to preserve |
...
| Runtime observation executes messaging consequences | Fusion reads recovery and mailbox state, persists recovery, observes quota reset, schedules attention work, emits events, paints chrome, and confirms delivery ACKs. | Verified responsibility leak |
