...
| Daemon status consumes one body-free messaging projection | `WorkspaceMessaging::status_snapshot` owns mailbox route fallback, unread counts, held attention, and the bounded blocked-wake sample. Status composition retains the legacy session-ledger fold as a separate compatibility half and no longer reads mailbox projections or notification indexes directly. | Verified behavior preserved by the Milestone 3 status-family pass |
| Durable operations cannot construct or traverse the daemon root | The `cyclopsd` composition root constructs `WorkspaceMessaging` and implements its named post-commit effects capability. The operation Module contains neither the daemon adapter nor an `Inner` constructor; a syntactic dependency lint protects that boundary. Runtime scheduling helpers that still accept `Arc<Inner>` remain explicit completion work. | Verified boundary plus remaining Milestone 3 work |
...
| Runtime evidence and post-commit work enter through the messaging Interface | Fusion and authenticated ACK handling publish immutable route evidence to `WorkspaceMessaging`. Delivery reports durable pre-write, attention, and notified outcomes, and the socket server reports an enabled force-submit setting. The Module decides route reconciliation, reminder, exact-attention reconciliation, and force-submit consequences; a syntactic lint prevents those callers from invoking messaging schedulers directly. The composition adapter still hosts the retained mechanisms. | Verified behavior preserved by the Milestone 3 runtime-evidence pass |
...
| Reusable UI code still owns backend mechanisms | `cyclops-ui` performs socket IO, direct bounded ledger backfill, and tmux focus. The workspace also owns a separate blocking bounded event reader. | Verified presentation seam leak |
...
| CI concerns are separate | Duplicate relocated runs, missing cancellation, interruption ownership, zero doctests, broad matrix/jobs, and missing retained performance history remain in current workflow structure. | Verified or partially verified as classified in section 14 |
...
|---|---|---|
| Frame-size P1 | Still present | The UI is internally bounded; daemon and blocking CLI ingress and egress remain uncapped. |
...
| Subscribe cursor | Still present with narrower impact | The type promises replay and the daemon ignores it, but public protocol prose and current UI callers do not rely on it. |
...
| Current CI timing and residue counts | Unverified as current measurements | Workflow structure was rechecked. Historical wall time, failure rate, and the review’s 8/8/37 residue snapshot were not refreshed. |
...
| P1 disposition after approval | Two documentation defects corrected; frame defect remains | Terminal wording and raw-tmux authority are repaired without production changes. The bounded frame contract is Milestone 1. |
