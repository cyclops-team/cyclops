...
| Runtime observation still coordinates several consequence domains | Milestone 4 moved positive quota-reset consequences behind an immutable pane observation consumed by `WorkspaceMessaging`, and the Milestone 3 recovery correction removed recovery projection and persistence from fusion. Fusion still reads active composer candidates for its runtime composer projection, schedules attention work, emits events, paints chrome, and confirms delivery ACKs. | Verified responsibility leak, narrowed by Milestones 3 and 4 |
