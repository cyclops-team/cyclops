...
| Runtime observation still coordinates one messaging consequence | Delivery ACK confirmation still follows inside the observer after the state event. | Verified responsibility leak; move the confirmed immutable candidates through the composition root in the next focused Milestone 4 correction |
