...
If you are installing Cyclops to use it, read [install.md](install.md)
instead. If you are trying to find your way around the code, read
[HANDOFF.md](HANDOFF.md). If you are about to change delivery, the ledger,
or anything that renders, read [INVARIANTS.md](INVARIANTS.md) first.
...
`demos/parity-check.sh` runs every command shape the README and `docs/`
...
`crates/cyclops-testrig`. Never `tmux` directly, never the default server,
...
- `crates/cyclops-testrig/tests/teardown_has_one_home.rs` fails if any
  other Rust file starts or kills a tmux server.
- `crates/cyclops-testrig/tests/shell_teardown.rs` holds `demos/lib.sh`,
  the shell home of the same rule, to the same contract.

Shell and Python go through `demos/lib.sh`. Source it, never paste from it.
...
- **[STYLE.md](STYLE.md) is binding**, on code, comments and docs. Read it
...
- **Wire changes are additive.** New fields are optional; unknown fields
  ...
  a version mismatch warns rather than rejects ([PROTOCOL.md](PROTOCOL.md)).
...
- **No polling.** If you are reaching for an interval, you have not found
  the event yet ([INVARIANTS.md](INVARIANTS.md), rule 9).
