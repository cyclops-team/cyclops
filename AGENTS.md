...
[docs/CONTRIBUTING.md](docs/CONTRIBUTING.md) — follow those. Non-obvious
...
- **Two `cyclops-theme` tests are timing-sensitive on this VM.**
  ...
  `crates/cyclops-theme/src/select.rs` rewrite a file to the same length
...
- `frontend/` is read-only branding (see above). `npm run dev` from
  `frontend/` serves it on port 5173 if you need to look; do not modify
