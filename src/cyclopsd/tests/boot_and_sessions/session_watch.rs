...
async fn watching_a_session_after_boot_makes_status_see_it() {
    ...
    loop {
        ...
        // No event: `session` announces detach, not the runtime row leaving
        // status.
...
async fn a_renamed_watched_session_keeps_one_slot_registry_and_ledger() {
    ...
    let renamed_status = loop {
        ...
        // Attach publishes `session` and a rename publishes `renamed`; wake on
        // either and ask status again. Neither `name` nor `attached` is masked
        // by the status refresh.
        rig.ev
            .wake_on(deadline.saturating_duration_since(Instant::now()), |e| {
                matches!(e["event"].as_str(), Some("session" | "renamed"))
            })
            .await;
    };
    assert_eq!(
