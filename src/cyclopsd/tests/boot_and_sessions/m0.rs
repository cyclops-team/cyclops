...
async fn m0_shadow_daemon_end_to_end() {
        ...
        .any(|l| l.starts_with("FIXPROMPT"))
    {
        ...
        // No event: the prompt is read from a tmux capture.
    ...
    let pane_id = loop {
        ...
        // Status, not events: this loop is the startup measurement, and the
        // subscription opens below.
    ...
    loop {
        ...
        // No event: `pane.read` is a direct read of one pane.
        tokio::time::sleep(Duration::from_millis(100)).await;
    }
