...
async fn test_three_agent_journey() {
    ...
    // Give a brief moment for the submitted doorbell turn to finish in the agent command loop
    ...
    // Verify Codey is observed as working
    let mut codey_is_working = false;
    for _ in 0..20 {
        let status = rig.ctl.request("status", json!({})).await;
        if let Some(panes) = status["result"]["sessions"][0]["panes"].as_array() {
            if let Some(p) = panes.iter().find(|x| x["pane_id"] == codey_pane) {
                if p["state"] == "working" {
                    codey_is_working = true;
                    break;
                }
            }
        }
    ...
    assert!(
        codey_is_working,
        "Codey should be detected in WORKING state"
    );
