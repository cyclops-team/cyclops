...
async fn tier1_hook_ack_and_late_upgrade() {
    ...
    wait_notification_state(&mut rig, &m1, &["submitted", "submitted_unverified"]).await;
    let resp = report_ack(&rig, "hooky", &m1, 1, "t1").await;
    assert_eq!(resp["applied"], true, "{resp}");
    assert_eq!(resp["matched"], true, "{resp}");
    wait_notification_state(&mut rig, &m1, &["notified"]).await;
...
async fn decline_aborts_when_the_modal_changes_between_keys() {
    ...
    // the doorbell landed.
    wait_notification_state(&mut rig, &msg_id, &["notified", "submitted_unverified"]).await;
...
async fn ack_deadlines_freeze_across_detach() {
    ...
    rig.ev
        ...
        .await;
    // Wall-clock by contract: nothing is published while detached.
    ...
    let state =
        wait_notification_state(&mut rig, &msg_id, &["notified", "attention_required"]).await;
    assert_eq!(
        state,
        "notified",
        "{:?}",
...
async fn a_second_message_waits_for_the_first_turn_and_then_lands() {
    ...
    wait_notification_state(&mut rig, &ids[0], &["notified", "submitted_unverified"]).await;
    claim(&rig, "worker", &ids[0]);
    wait_notification_state(&mut rig, &ids[1], &["notified", "submitted_unverified"]).await;
...
async fn escaped_capture_flips_typed_text_to_idle_with_input_and_gates() {
    ...
    rig.tmux.run_ok(&["send-keys", "-t", &pane, "x", "Enter"]);
    wait_pane_state(&mut rig, "idle").await;
    // A bounded window: nothing marks a paste that must not happen.
...
async fn pane_rebound_before_submit_withholds_the_submit_key() {
    ...
    loop {
        ...
        // No event: the pane command is read from tmux.
        tokio::time::sleep(Duration::from_millis(50)).await;
    }
    release.add_permits(1);
    ...
    // A bounded window: nothing marks a submit that must not happen.
...
async fn quiesce_is_quiet_over_prepaste_notifications() {
    ...
    let (result, _) = rig
        ...
        .await;
    let msg_id = result["msg_id"].as_str().unwrap().to_string();
    wait_notification_state(&mut rig, &msg_id, &["gating"]).await;
