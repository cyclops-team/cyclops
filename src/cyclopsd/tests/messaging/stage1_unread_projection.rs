...
async fn wait_for_option(rig: &Rig, pane: &str, expected: &str) {
    ...
    while Instant::now() < deadline {
        ...
        // No event: the option is read back from tmux.
...
async fn wait_for_border_needle(rig: &Rig, pane: &str, needle: &str, should_contain: bool) {
    ...
    while Instant::now() < deadline {
        ...
        // No event: the border is read back from tmux.
        tokio::time::sleep(Duration::from_millis(50)).await;
    }
    let text = border_text(rig, pane);
    panic!(
...
async fn unadopt_is_the_last_chrome_writer_after_an_in_flight_unread_projection() {
    ...
    let response = {
        ...
        tokio::select! {
            ...
            // A negative bound: the request must still be blocked after this window.
    ...
    // A bounded window: nothing marks a repaint that must not happen.
...
async fn shutdown_joins_the_unread_worker_before_restoring_user_chrome() {
    ...
    tokio::select! {
        ...
        // A negative bound: shutdown must still be held after this window.
        _ = tokio::time::sleep(Duration::from_millis(50)) => {}
    }
...
async fn slice1_supersession_and_unroutable_unread() {
    ...
    let resp = rig
        ...
        .await;
    assert_eq!(resp["result"]["label"], "worker", "{resp}");
    // Copy-mode holds the wake in the gate. A notification that has started
    // writing cannot be superseded, and a bare `cat` pane is written the
    // moment the gate admits it, so without the hold the supersede below
    // races the delivery worker.
