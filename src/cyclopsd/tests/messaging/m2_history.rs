...
async fn history_reconstructs_a_two_pane_conversation() {
    ...
    wait_notification_state(&mut rig, &m1, &["notified", "submitted_unverified"]).await;
    claim(&rig, "reviewer", &m1);
    wait_notification_state(&mut rig, &m2, &["notified", "submitted_unverified"]).await;
    claim(&rig, "codex", &m2);
    ...
    loop {
        ...
        // No event this loop can trust: status answers `unknown` for every
        // pane while its live refresh is incomplete.
