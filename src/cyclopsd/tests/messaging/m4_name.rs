...
async fn overlapping_state_observation_waits_for_prior_chrome_repaint() {
    ...
    loop {
        ...
        // inside its own budget; this waits for a complete answer, not a
        // state change.
...
async fn chrome_off_writes_no_tmux_option() {
    ...
    // A bounded window: nothing the daemon publishes marks a write that
    // must not happen.
...
async fn killing_a_watched_session_releases_its_labels() {
    ...
    loop {
        ...
        // No event: the released label shows only as the refusal going away.
