...
impl ControlClient {
    ...
    /// reply may itself be uncertain. Callers must reconcile host state after
    /// every attempted operation.
    pub async fn close_session_at(
        ...
        self.command(&format!("kill-session -t {}", quote_arg(target_session_id)))
            .await
            .map(|_| ())
    }
}
