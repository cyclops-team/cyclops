...
//! `src/cyclops-workspace/src/sync.rs`'s `hydrate_visible_tab`.
...
/// `src/cyclops-workspace/tests/baseline.rs`'s rationale).
#[tokio::test]
async fn timing_serial_vs_concurrent_hydration_on_eight_panes() {
    ...
    // src/cyclops-workspace/tests/baseline.rs's own 8-pane fixture uses
    // 220x50.
    srv.tmux_ok(&[
        "new-session",
        "-d",
