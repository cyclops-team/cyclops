...
- `frontend/static/install.sh`, the public `curl -fsSL https://www.usecyclops.dev/install.sh | sh` bootstrap that fetches the release from GitHub, runs it through the existing network-free `commPact-install`, and links `cyclops` onto `PATH`.
